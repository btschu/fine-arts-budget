...
export default function ExpenseLedger({
  ...
  if (rows.length === 0) {
    return (
      <p className="rounded-lg border border-dashed border-slate-300 p-6 text-center text-sm text-slate-500 dark:border-neutral-700 dark:text-neutral-400">
    ...
    <>
      ...
      <div className="no-print mb-3 flex flex-wrap items-center gap-2">
        <input
          ...
          className="min-w-0 flex-1 rounded border border-slate-300 px-3 py-1.5 text-sm dark:border-neutral-700 dark:bg-neutral-800 dark:text-neutral-100"
        />
        <button
          type="button"
          onClick={() =>
      ...
      {filteredRows.length === 0 ? (
        ...
        <>
          ...
          {/* Desktop: table */}
          <div className="hidden rounded-lg border border-slate-200 bg-white shadow-sm lg:block dark:border-neutral-800 dark:bg-neutral-900">
            <table className="w-full text-sm">
              <thead>
                <tr className="border-b border-slate-200 bg-slate-50 text-left text-xs font-medium uppercase tracking-wide text-slate-500 dark:border-neutral-800 dark:bg-neutral-800 dark:text-neutral-400">
  ...
  return (
    <>
      <div className="flex flex-col gap-1">
        <label className="text-xs text-slate-500 dark:text-neutral-400">Purchased from</label>
        <input
          name="vendor"
          defaultValue={row.vendor}
          required
          list={VENDOR_LIST_ID}
          className="rounded border border-slate-300 px-2 py-1 text-sm dark:border-neutral-700 dark:bg-neutral-800 dark:text-neutral-100"
        />
      </div>
      <div className="flex flex-col gap-1">
        <label className="text-xs text-slate-500 dark:text-neutral-400">Item</label>
        <input
          name="item"
      ...
      {categories.length > 0 && (
        <div className="flex flex-col gap-1">
          <label className="text-xs text-slate-500 dark:text-neutral-400">Category</label>
          <select
            name="categoryId"
      ...
      <div className="flex flex-col gap-1">
        <label className="text-xs text-slate-500 dark:text-neutral-400">Cost</label>
        <input
          name="amount"
          type="number"
          step="0.01"
          ...
          required
          className="w-full rounded border border-slate-300 px-2 py-1 text-sm dark:border-neutral-700 dark:bg-neutral-800 dark:text-neutral-100"
        />
      </div>
      <div className="flex flex-col gap-1">
        <label className="text-xs text-slate-500 dark:text-neutral-400">Date of purchase</label>
        <input
          name="spentAt"
          type="date"
      ...
      <div className="flex flex-col gap-1">
        ...
        <input
          ...
          type="file"
          accept="image/*"
          className="text-xs text-slate-500 file:mr-2 file:rounded file:border-0 file:bg-slate-100 file:px-2 file:py-1 file:text-xs dark:text-neutral-400 dark:file:bg-neutral-800 dark:file:text-neutral-300"
        />
      </div>
    </>
  );
...
function ReceiptLink({ id }: { id: string }) {
  return (
    <a
      ...
      className="inline-block rounded border border-slate-300 px-2 py-1 text-xs text-slate-600 hover:bg-slate-50 dark:border-neutral-700 dark:text-neutral-400 dark:hover:bg-neutral-800"
    >
      📎 Receipt
    </a>
  );
  ...
  return (
    <div className="rounded-lg border border-slate-200 bg-white p-4 shadow-sm dark:border-neutral-800 dark:bg-neutral-900">
      <div className="flex items-start justify-between gap-2">
        <div>
          <p className="font-medium text-slate-900 dark:text-neutral-100">{row.vendor}</p>
          <p className="text-xs text-slate-500 dark:text-neutral-400">
            {formatDate(row.spentAt)} · {row.item}
          </p>
        </div>
        <p className="whitespace-nowrap text-right font-semibold text-slate-900 dark:text-neutral-100">
      ...
      {!readOnly &&
        (confirmingDelete ? (
          <div className="mt-3 flex items-center justify-between gap-2 border-t border-slate-100 pt-3 dark:border-neutral-800">
            <span className="text-xs text-slate-600 dark:text-neutral-400">Delete this purchase?</span>
          ...
          <div className="mt-3 flex justify-end gap-2 border-t border-slate-100 pt-3 dark:border-neutral-800">
            <button
              type="button"
...
function ExpenseRow({
  ...
  if (editing) {
    return (
      <tr className="border-b border-slate-100 bg-amber-50/40 dark:border-neutral-800 dark:bg-amber-950/20">
  ...
  return (
    <tr className="border-b border-slate-100 last:border-0 hover:bg-slate-50 dark:border-neutral-800 dark:hover:bg-neutral-800/60">
      <td className="px-4 py-2 whitespace-nowrap text-slate-600 dark:text-neutral-400">
      ...
      <td className="px-4 py-2 text-slate-600 dark:text-neutral-400">{row.vendor}</td>
      <td className="px-4 py-2">
        <CategoryTag categoryName={row.categoryName} categoryColor={row.categoryColor} />
      </td>
      <td className="px-4 py-2 text-slate-600 dark:text-neutral-400">
        {row.enteredByName}
        {row.updatedByName && (
          <span className="text-xs text-slate-400 dark:text-neutral-500"> (edited by {row.updatedByName})</span>
        )}
      </td>
      <td className="px-4 py-2 text-right text-slate-900 dark:text-neutral-100">
        {formatCurrency(row.amount)}
      </td>
      <td className="px-4 py-2 text-right text-slate-600 dark:text-neutral-400">
        ...
        <td className="px-4 py-2 text-right whitespace-nowrap">
          {confirmingDelete ? (
            <span className="flex items-center justify-end gap-2">
              <span className="text-xs text-slate-600 dark:text-neutral-400">Delete?</span>
              <button
                type="button"
                disabled={deletePending}
                onClick={confirmDelete}
